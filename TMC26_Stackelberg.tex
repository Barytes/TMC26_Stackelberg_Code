...
In practice, the MEC ecosystem involves multiple self-interested entities. The computation resources at the edge server are provided and managed by Edge Service Providers (ESPs) (\eg, AWS Wavelength and Huawei Intelligent EdgeCloud \etc). Concurrently, the wireless network resources required for data transmission are managed by Network Service Providers (NSPs), (\eg, China Telecom and AT\&T). Both ESPs and NSPs are profit-driven entities that aim to maximize their revenues by strategically pricing their respective resources. Meanwhile, mobile users are also rational and self-interested. They compete for limited computation and network resources to minimize their individual costs, which comprise execution delay, energy consumption, and payments to the service providers.

While extensive research has investigated task offloading, resource allocation in MEC \cite{wang2025trust,wei2024quantum,dai2025throughput,hu2020heterogeneous,chen2024multi,chu2023efficient,chen2024price,tong2024stackelberg,wang2022monetizing,ding2023optimal,tutuncuoglu2024joint,wang2022profit,liu2022resource,wang2023marginal}, the majority of these works focus on optimizing system-wide metrics \cite{wang2025trust,wei2024quantum,dai2025throughput}, neglecting the strategic behaviors of the entities. Some game-theoretic studies have investigated users' strategic offloading competition \cite{hu2020heterogeneous,chen2024multi,chu2023efficient}, but they typically consider simplified user decisions, such as binary offloading, subtask-level decisions, or competition over a single resource dimension. Another line of work studies pricing mechanisms in MEC \cite{chen2024price,tong2024stackelberg,wang2022monetizing,ding2023optimal,tutuncuoglu2024joint,wang2022profit,liu2022resource,wang2023marginal}. However, these studies either consider a single service provider or a centralized operator jointly managing multiple resources \cite{tong2024stackelberg,wang2022monetizing,tutuncuoglu2024joint,wang2023marginal}, or involve multiple providers without explicitly capturing the strategic interaction between independent providers of complementary computation and network resources \cite{chen2024price,ding2023optimal,wang2022profit,liu2022resource}. To the best of our knowledge, the strategic interplay between an ESP and an NSP offering \textit{complementary} resources (\ie, computation and bandwidth), together with users' selfish offloading competition under coupled computation and bandwidth constraints, remains unexplored.

Capturing such interactions presents significant technical challenges. First, due to the coupled resource capacity constraints, users' offloading strategies are highly interdependent, forming a Generalized Nash Equilibrium Problem (GNEP). We prove that computing the Socially Optimal Equilibrium (SOE) for users is fundamentally NP-hard. Second, the service providers' pricing game heavily relies on the users' equilibrium response. Because the users' equilibrium offloading user set changes discontinuously with prices and lacks a closed-form expression, the providers' revenues cannot be explicitly formulated. This makes traditional continuous game-theoretic approaches and bilevel optimization techniques (such as MPEC or EPEC) computationally intractable.
...
Algorithm~\ref{alg:brgd} terminates at a pair $(\mathcal{X}^*,\boldsymbol{p}^*)$ for which no candidate offloading user set in the local search region yields a smaller deviation gap. In particular, at termination we have
$\epsilon(\mathcal{Y}) \ge \epsilon(\mathcal{X}^*)$ for all
$\mathcal{Y} \in \{\mathcal{Y}_E^*(\boldsymbol{p}^*),\mathcal{Y}_N^*(\boldsymbol{p}^*)\}\cup \mathcal{N}(\boldsymbol{p}^*)$.
Hence, $(\mathcal{X}^*,\boldsymbol{p}^*)$ is a local optimum under the neighborhood structure induced by $\mathcal{N}(\boldsymbol{p}^*)$.
Moreover, the achieved gap
$\epsilon(\mathcal{X}^*)=\max\{\hat G_E(\boldsymbol{p}^*),\hat G_N(\boldsymbol{p}^*)\}$ upper bounds the maximum unilateral deviation gain of either provider at $\boldsymbol{p}^*$, and the output hence constitutes an $\epsilon(\mathcal{X}^*)$-NE by Definition~\ref{eps-ne}.

The per-iteration cost is dominated by the RNE subroutine in Algorithm~\ref{alg:optimal_rne}, which evaluates $L$ boundary points and invokes Algorithm~\ref{alg:brgd} at each point. By Proposition~\ref{prop:gain-complexity}, each call to Algorithm~\ref{alg:brgd} runs in $O(|\mathcal{I}|^3)$ time. In an outer iteration of Algorithm~\ref{alg:brgd}, the initialization stage evaluates at most two candidate sets, and the neighborhood stage---which is triggered only when no improvement is found in the initialization---enumerates $|\mathcal{N}(\bm p)|=O(|\mathcal{I}|^2)$ candidates. Therefore, the worst-case per-iteration complexity is $O(L\cdot|\mathcal{I}|^5)$. Although the worst-case iteration count can be exponential in principle, prioritizing the exact deviation targets of the providers early in each iteration yields a strongly directed search in practice, and we observe convergence within a small number of iterations in our experiments.
...
\begin{itemize}
    ...
    \item \textbf{Variational Inequality Baseline (VI):} 
    We adopt a shared-multiplier \emph{variational equilibrium} baseline for the Stage-II generalized Nash equilibrium problem. 
    For any given offloading set, the coupled capacity constraints are handled via common shadow prices, and each offloading user admits a closed-form optimal demand as a function of these prices. 
    We then update the shadow prices by projected dual ascent to satisfy the computation and bandwidth capacity constraints, and finally apply a simple feasibility repair on the offloading set if necessary.

    \item \textbf{Penalty Best-Response Baseline (PEN):} 
    We implement a penalty-based sequential best-response baseline by relaxing the coupled capacity constraints into users' objectives through quadratic penalty terms. 
    Given others' current demands, each user updates its offloading decision and resource demands by minimizing a penalized individual cost (which reduces to two one-dimensional convex subproblems for computation and bandwidth). 
    The penalty coefficient is increased progressively to enforce feasibility, and a lightweight repair step is applied if residual capacity violations remain.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Stage II (User-side SOE computation).}
  \begin{itemize}
    \item \textbf{(S2-1) Social-cost convergence.}
    \begin{itemize}
      \item \emph{x-axis:} iteration index (outer iteration if nested).
      \item \emph{y-axis:} total social cost $\sum_{i\in\mathcal I} C_i(\cdot)$ (or the potential / social cost objective).
      \item \emph{Baselines:} centralized solver (small $|\mathcal I|$ only), greedy-only / random feasible solution.
      \item \emph{Plot:} line plot (multiple curves).
    \end{itemize}

    \item \textbf{(S2-2) Exploitability / equilibrium violation convergence.}
    \begin{itemize}
      \item \emph{x-axis:} iteration index.
      \item \emph{y-axis:} average exploitability
      $
      \frac1{|\mathcal I|}\sum_{i\in\mathcal I}\bigl(C_i(\text{current})-C_i(\text{BR}_i)\bigr)_+
      $
      (and/or max exploitability).
      \item \emph{Baselines:} centralized solver (small $|\mathcal I|$), greedy-only, random feasible.
      \item \emph{Plot:} line plot.
    \end{itemize}

    \item \textbf{(S2-3) Approximation gap vs. system size (validating Theorem~2).}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$ (small-scale regime where the optimum is computable).
      \item \emph{y-axis:} relative gap
      $
      \frac{V_{\text{alg}}-V_{\text{opt}}}{V_{\text{opt}}}\times 100\%
      $.
      \item \emph{Baselines:} centralized solver as $V_{\text{opt}}$; optionally a naive greedy baseline.
      \item \emph{Plot:} bar plot or line plot with error bars (across random instances).
    \end{itemize}

    \item \textbf{(S2-4) Wall-clock runtime scalability.}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$ (e.g., $10\rightarrow 100\rightarrow 500$).
      \item \emph{y-axis:} runtime in seconds (wall-clock).
      \item \emph{Baselines:} centralized solver (until timeout), greedy-only baseline.
      \item \emph{Plot:} (semi-)log line plot.
    \end{itemize}

    \item \textbf{(S2-5, optional) Offloading ratio / $|X|$.}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$ or resource capacities $(F,B)$ / ratio $F/B$.
      \item \emph{y-axis:} $|X|/|\mathcal I|$ (or $|X|$).
      \item \emph{Baselines:} centralized solver (small $|\mathcal I|$), random offloading, heuristic thresholding.
      \item \emph{Plot:} line plot or grouped bars.
    \end{itemize}

    \item \textbf{(S2-6, optional) Decomposition ablation (Alg.~1 vs Alg.~2 contributions).}
    \begin{itemize}
      \item \emph{x-axis:} algorithm variants (Alg.~1 only / Alg.~2+Alg.~1 / Alg.~2 without inner refinement).
      \item \emph{y-axis:} social cost and/or exploitability.
      \item \emph{Baselines:} variant comparison (plus centralized solver if feasible).
      \item \emph{Plot:} grouped bar plot.
    \end{itemize}
  \end{itemize}

  \item \textbf{Stage I (Pricing equilibrium / Stackelberg computation).}
  \begin{itemize}
    \item \textbf{(S1-1) Deviation-gap certificate vs. iteration.}
    \begin{itemize}
      \item \emph{x-axis:} outer iteration index of Algorithm~5.
      \item \emph{y-axis:} deviation gap / certificate
      $
      \epsilon^{(t)}=\max\{\hat G_E(\bm p^{(t)}),\hat G_N(\bm p^{(t)})\}
      $.
      \item \emph{Baselines:} Algorithm~5, provider best-response dynamics (PBRD), Bayesian optimization (BO), DRL; grid search oracle (GSO) for small instances.
      \item \emph{Plot:} line plot.
    \end{itemize}

    \item \textbf{(S1-2) Final deviation gap vs. $|\mathcal I|$.}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$.
      \item \emph{y-axis:} final $\epsilon$ (or final deviation gap).
      \item \emph{Baselines:} Algorithm~5,  PBRD, BO, DRL, (GSO for small $|\mathcal I|$).
      \item \emph{Plot:} line plot or grouped bars.
    \end{itemize}

    \item \textbf{(S1-3) Runtime vs. $|\mathcal I|$ (pricing-level scalability).}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$.
      \item \emph{y-axis:} wall-clock time (seconds).
      \item \emph{Baselines:} Algorithm~5, PBRD, BO, DRL, (GSO small-scale).
      \item \emph{Plot:} (semi-)log line plot.
    \end{itemize}

    \item \textbf{(S1-4) Search budget: \# evaluated price points / \# Stage-II calls.}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$.
      \item \emph{y-axis:} (i) number of evaluated price points, and/or (ii) number of Stage-II solver calls.
      \item \emph{Baselines:} Algorithm~5, PBRD, BO, (GSO).
      \item \emph{Plot:} bar plot or line plot.
    \end{itemize}

    \item \textbf{(S1-5, optional) Price trajectory in the $(p_E,p_N)$ plane.}
    \begin{itemize}
      \item \emph{x-axis:} $p_E$; \emph{y-axis:} $p_N$.
      \item \emph{Baselines:} Algorithm~5 (optionally overlay BGSS/PDSS).
      \item \emph{Plot:} 2D trajectory (scatter + arrows).
    \end{itemize}

    \item \textbf{(S1-6, optional) Landscape visualization (small instance).}
    \begin{itemize}
      \item \emph{x-axis:} $p_E$; \emph{y-axis:} $p_N$.
      \item \emph{Color:} $\epsilon(p_E,p_N)$ (or $\max\{G_E,G_N\}$).
      \item \emph{Baselines:} none (overlay Algorithm~5 trajectory).
      \item \emph{Plot:} heatmap / contour.
    \end{itemize}
  \end{itemize}

  \item \textbf{Strategic-setting comparisons (system-level outcomes).}
  \begin{itemize}
    \item \textbf{(ST-1) Total user social cost across strategic settings.}
    \begin{itemize}
      \item \emph{x-axis:} system load (e.g., $|\mathcal I|$ or mean $w_i$).
      \item \emph{y-axis:} total user social cost.
      \item \emph{Baselines:} Full model (Alg.~5 + Alg.~1/2), Market Equilibrium, SingleSP, Fixed pricing, Random offloading.
      \item \emph{Plot:} line plot.
    \end{itemize}

    \item \textbf{(ST-2) Total provider revenue $U_E+U_N$ across settings.}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$ or $(F,B)$ / $F/B$.
      \item \emph{y-axis:} $U_E+U_N$.
      \item \emph{Baselines:} same as ST-1.
      \item \emph{Plot:} line plot.
    \end{itemize}

    \item \textbf{(ST-3) Provider split: ESP revenue $U_E$ and NSP revenue $U_N$.}
    \begin{itemize}
      \item \emph{x-axis:} $F/B$ ratio (recommended) or $|\mathcal I|$.
      \item \emph{y-axis:} $U_E$ (and separately $U_N$).
      \item \emph{Baselines:} same as ST-1.
      \item \emph{Plot:} two line plots (or one figure with two subplots).
    \end{itemize}

    \item \textbf{(ST-4, optional) Resource utilization (showing complementarity).}
    \begin{itemize}
      \item \emph{x-axis:} $|\mathcal I|$ or $F/B$.
      \item \emph{y-axis:} computation utilization $\sum_i f_i/F$ and bandwidth utilization $\sum_i b_i/B$.
      \item \emph{Baselines:} Full model, SingleSP, Fixed pricing.
      \item \emph{Plot:} two line plots (or two subplots).
    \end{itemize}
  \end{itemize}

  \item \textbf{Ablation studies.}
  \begin{itemize}
    \item \textbf{(AB-1) Sampling density $L$ vs. equilibrium accuracy.}
    \begin{itemize}
      \item \emph{x-axis:} $L$ (number of sampled boundary directions / points).
      \item \emph{y-axis:} final deviation gap $\epsilon$.
      \item \emph{Baselines:} none (or multiple curves for different $|\mathcal I|$).
      \item \emph{Plot:} line plot.
    \end{itemize}

    \item \textbf{(AB-2) Sampling density $L$ vs. computational cost.}
    \begin{itemize}
      \item \emph{x-axis:} $L$.
      \item \emph{y-axis:} runtime and/or \# Stage-II calls.
      \item \emph{Baselines:} none.
      \item \emph{Plot:} line plot (often paired with AB-1 as two subplots).
    \end{itemize}

    \item \textbf{(AB-3, optional) Neighborhood / initialization sensitivity.}
    \begin{itemize}
      \item \emph{x-axis:} variants (neighborhood on/off; different initializations).
      \item \emph{y-axis:} final $\epsilon$ and runtime.
      \item \emph{Baselines:} variant comparison.
      \item \emph{Plot:} box plot or grouped bars.
    \end{itemize}
  \end{itemize}
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/esp_potential_revenue_contour.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/nsp_potential_revenue_contour.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/potential_offload_ratio_contour.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/algorithm2_exploitability_vs_users.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/stage2_wall_time_vs_users.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/stage2_solver_comparison_vs_users.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/algorithm2_social_cost_gap_vs_users.png}
    \caption{Enter Caption}
    \label{fig:placeholder}
\end{figure}
