#!/usr/bin/env python3
"""Dedicated Stage-II solver comparison: social cost vs number of users.
...
This script is intentionally standalone and focused on one figure/table family.
`CS` is evaluated only when `n_users <= exact_max_users`; otherwise it is skipped
for that user-count and recorded as skipped in the run metadata.
"""
...
def _write_summary_csv(rows: list[dict[str, object]], out_path: Path) -> list[dict[str, float | int | str]]:
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for (n_users, method) in sorted(grouped):
            ...
            social_mean, social_std = _mean_std(social)
            off_mean, off_std = _mean_std(off)
            row = {
                "n_users": n_users,
                "method": method,
                "social_cost_mean": social_mean,
                "social_cost_std": social_std,
...
def _plot(summary_rows: list[dict[str, float | int | str]], out_path: Path) -> None:
    ...
    for method in METHODS:
        ...
        y = [float(row["social_cost_mean"]) for row in rows]
        yerr = [float(row["social_cost_std"]) for row in rows]
        style = STYLE[method]
        ax.errorbar(
            x,
            y,
    ...
    ax.set_title("Stage-II Solver Comparison: Social Cost vs Number of Users")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage-II solver comparison vs number of users")
    parser.add_argument("--config", type=str, default="configs/default.toml")
    parser.add_argument(
        "--user-counts",
        type=str,
        default="8,10,12",
        help="Comma-separated user counts. Defaults are small so CS is included by default.",
    )
    ...
    if not args.run_name:
        ...
        run_name = f"stage2_solver_comparison_vs_users_{timestamp}"
    else:
    ...
    _write_raw_csv(raw_rows, run_dir / "raw_stage2_solver_comparison_vs_users.csv")
    summary_rows = _write_summary_csv(raw_rows, run_dir / "summary_stage2_solver_comparison_vs_users.csv")
    _plot(summary_rows, run_dir / "stage2_solver_comparison_vs_users.png")
