#!/usr/bin/env python3
"""Stage-II wall-clock runtime vs number of users.

Compares physical execution time (wall-clock time) for:
- Algorithm 2
- UBRD
- VI
- PEN
...
def _write_raw_csv(rows: list[dict[str, object]], out_path: Path) -> None:
    fields = ["trial", "seed", "n_users", "method", "runtime_sec"]
...
def _write_summary_csv(rows: list[dict[str, object]], out_path: Path) -> list[dict[str, float | int | str]]:
    ...
    fields = ["n_users", "method", "runtime_sec_mean", "runtime_sec_std", "count"]
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for (n_users, method) in sorted(grouped):
            ...
            times = [float(x["runtime_sec"]) for x in bucket]
            t_mean, t_std = mean_std(times)
            row = {
                "n_users": n_users,
                "method": method,
                "runtime_sec_mean": t_mean,
                "runtime_sec_std": t_std,
                "count": len(bucket),
            }
...
def _plot(summary_rows: list[dict[str, float | int | str]], out_path: Path) -> None:
    ...
    for method in METHODS:
        ...
        y = [float(row["runtime_sec_mean"]) for row in rows]
        yerr = [float(row["runtime_sec_std"]) for row in rows]
        style = STYLE[method]
        ax.errorbar(
            x,
            y,
    ...
    ax.set_title("Stage-II Wall-clock Time vs Number of Users")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage-II wall-clock time vs number of users")
    parser.add_argument("--config", type=str, default="configs/default.toml")
    parser.add_argument("--user-counts", type=str, default="10,20,50,100,200,500")
    ...
    if not args.run_name:
        ...
        run_name = f"stage2_wall_time_vs_users_{timestamp}"
    else:
    ...
    _write_raw_csv(raw_rows, run_dir / "raw_stage2_wall_time_vs_users.csv")
    summary_rows = _write_summary_csv(raw_rows, run_dir / "summary_stage2_wall_time_vs_users.csv")
    _plot(summary_rows, run_dir / "stage2_wall_time_vs_users.png")
    ...
    meta_lines = [
        ...
        "metric = physical wall-clock time measured by perf_counter",
    ]
